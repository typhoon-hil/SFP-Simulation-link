---------------------------------------------------------------------------------------------------------------------------------------
--
--  Module Name: aurora_wrapper
--  Description:
--           Module wraps up all aurora realted modules. The module implementation follows Xilinx notation to
--           allow easier core modifications.
--
---------------------------------------------------------------------------------------------------------------------------------------
library ieee;
...
architecture rtl of aurora_wrapper_1l is
  ...
  signal vio_watch_au_wrap_reset    : std_logic_vector(0 downto 0);
  signal vio_watch_au_wrap_gt_reset : std_logic_vector(0 downto 0);
  signal vio_watch_i_axi_tx_data    : std_logic_vector(31 downto 0);
  signal set_vio_reset              : std_logic_vector(0 downto 0);
  signal set_vio_gt_reset           : std_logic_vector(0 downto 0);
...
begin
  ...
  --     gtx_ext_rst_d <= i_gtx_rst_n;
  --     gtx_ext_rst_dd <= gtx_ext_rst_d;
  --     gtx_ext_rst_ddd <= gtx_ext_rst_dd;
  --     gtx_ext_rst_dddd <= gtx_ext_rst_ddd;
  --     gtx_ext_rst_ddddd <= gtx_ext_rst_dddd;
  --     gtx_ext_rst_dddddd <= gtx_ext_rst_ddddd;
  --     gtx_ext_rst_ddddddd <= gtx_ext_rst_dddddd;
  --   end if;
  -- end process;

  -- process(user_clk_i, i_reset_n)
  -- begin
  --   if (i_reset_n = '0') then 
  --     system_reset_in <= '1';
  --   elsif(rising_edge(user_clk_i)) then
  --       if(gtx_ext_rst_ddddddd = '1') then
  --         system_reset_in <= '0';
  --       else
  --         system_reset_in <= '1';
  --       end if;
  ...
  -- end process;
    ...
    -- generate_gtx_reset:process(i_init_clk)
    --   begin
    --   if (rising_edge(i_init_clk)) then
    --     if (i_reset_n = '1') then
    --       if (gtx_rst_cnt_tc = '1') then
    --         gtx_rst_n <= '1';
    --       else
    ...
    --       end if;
    --     else
    --       gtx_rst_cnt <= (others => '0');
    --     end if;
    --   end if;
    -- end process;
    -- gtx_rst_cnt_tc <= '1' when gtx_rst_cnt(3) = '1' else 
    --                   '0' when i_reset_n = '1';
  ...
  port map (
    ...
    reset               => au_wrap_reset,     -- Before vio : reset               
    gt_reset            => au_wrap_gt_reset,  -- Before vio: gtx_rst_n_dddddd                 
  ...
  au_wrap_reset    <= reset or set_vio_reset(0);
  au_wrap_gt_reset <= gtx_rst_n_dddddd or set_vio_gt_reset(0);

  my_vio_inst : my_vio
  PORT MAP (
    clk        => i_init_clk,
    probe_in0  => vio_watch_au_wrap_reset,
    probe_in1  => vio_watch_au_wrap_gt_reset,
    probe_in2  => vio_watch_i_axi_tx_data,
    probe_out0 => set_vio_reset,
    probe_out1 => set_vio_gt_reset
  );

  vio_watch_au_wrap_reset(0)    <= au_wrap_reset;
  vio_watch_au_wrap_gt_reset(0) <= au_wrap_gt_reset;
  vio_watch_i_axi_tx_data       <= i_axi_tx_data;
end rtl;
